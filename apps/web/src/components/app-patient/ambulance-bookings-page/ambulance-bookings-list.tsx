...
import {
  useCancelAmbulanceBooking,
  useMyAmbulanceBookings,
  useRescheduleAmbulanceBooking,
} from "@/hooks/queries/use-ambulances";
import {
  AmbulanceIcon,
  Calendar01Icon,
  Call02Icon,
...
function BookingCard({ booking }: { booking: any }) {
  const parsedDateTime = booking.date_time ? parseISO(booking.date_time) : null;
  const dateTime = (parsedDateTime && isValid(parsedDateTime))
    ? format(parsedDateTime, "EEEE, dd MMM yyyy • hh:mm a")
    : "-";

  const [rescheduleOpen, setRescheduleOpen] = useState(false);
  const cancelMutation = useCancelAmbulanceBooking();
  const rescheduleMutation = useRescheduleAmbulanceBooking();
  ...
  return (
    <Card className="bg-card border-border/50 rounded-md border p-4 shadow-xs transition-all duration-300">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:gap-8">
        ...
        <div className="flex items-center gap-3.5 lg:w-1/4">
          <div className="bg-primary/10 text-primary flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl">
            <HugeiconsIcon icon={AmbulanceIcon} size={28} />
          </div>
          <div className="min-w-0">
            <h3 className="text-foreground truncate text-sm font-bold">
              {booking.ambulance_type || "Ambulance"}
            </h3>
            {booking.isRoundTrip && (
        ...
        <div className="lg:border-border/40 grid grow grid-cols-1 gap-x-5 gap-y-4 sm:grid-cols-2 lg:grid-cols-4 lg:border-l lg:pl-8">
          <InfoField icon={Calendar01Icon} label="Date & Time" value={dateTime} />
          <InfoField icon={Location01Icon} label="From" value={booking.from_address} />
          <InfoField icon={Location01Icon} label="To" value={booking.to_address} />
        ...
        <div className="lg:border-border/40 flex shrink-0 items-center justify-between gap-3 border-t pt-4 sm:justify-start lg:border-t-0 lg:border-l lg:pl-8 lg:pt-0">
          ...
          <div className="flex items-center gap-2">
            <BookingActions
              ...
              cancelConfirmTitle="Cancel ambulance booking?"
              cancelConfirmDescription="This will cancel your ambulance request. You can always book again."
            />
          </div>
        </div>
      </div>
      ...
      <RescheduleDialog
        ...
        title="Reschedule ambulance"
        description="Pick a new date and time for your ambulance."
        initialDate={booking.date_time}
        isSubmitting={rescheduleMutation.isPending}
        onConfirm={(iso) =>
          rescheduleMutation.mutate(
            { id: booking._id, date_time: iso },
...
export function AmbulanceBookingsList() {
  const { data: bookings = [], isLoading } = useMyAmbulanceBookings();
  ...
  return (
    <Tabs defaultValue="upcoming" className="w-full space-y-6">
      ...
      <TabsContent value="upcoming">
        {upcoming.length === 0 ? (
          <EmptyState message="No upcoming ambulance bookings." />
        ) : (
          <div className="flex flex-col gap-4">
            {upcoming.map((b: any) => (
      ...
      <TabsContent value="completed">
        {completed.length === 0 ? (
          <EmptyState message="No completed ambulance bookings." />
        ) : (
          <div className="flex flex-col gap-4">
            {completed.map((b: any) => (
              <BookingCard key={b._id} booking={b} />
      ...
      <TabsContent value="cancelled">
        {cancelled.length === 0 ? (
          <EmptyState message="No cancelled ambulance bookings." />
        ) : (
          <div className="flex flex-col gap-4">
            {cancelled.map((b: any) => (
              <BookingCard key={b._id} booking={b} />
