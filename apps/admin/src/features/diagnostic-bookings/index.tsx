...
import { MoreHorizontal, Edit2, Trash2, Search as SearchIcon, FilterX, MapPin, Phone, User, Microscope, Banknote, UserCheck } from 'lucide-react'
...
export default function DiagnosticBookings() {
  ...
  const { data: bookingsData, isLoading } = useQuery({
    queryKey: ['diagnostic-bookings', searchTerm, statusFilter, page, limit],
    queryFn: async () => {
      ...
      const res = await api.get(`/diagnostic-bookings?${params.toString()}`)
      return res.data
    }
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/diagnostic-bookings/${id}`),
    onSuccess: () => {
      toast.success('Booking deleted')
      queryClient.invalidateQueries({ queryKey: ['diagnostic-bookings'] })
    },
    onError: () => {
    ...
    <>
      ...
      <Main className='flex flex-1 flex-col gap-4 sm:gap-6'>
        <div className='flex flex-wrap items-end justify-between gap-2'>
          <div>
            <h2 className='text-2xl font-bold tracking-tight'>Diagnostic Bookings</h2>
            <p className='text-muted-foreground'>
              Manage home collection requests and assign people for tests.
            </p>
          </div>
        </div>
          ...
          <Table>
            <TableHeader className='bg-white/5'>
              <TableRow className='hover:bg-transparent border-white/5'>
                <TableHead className='ps-6 py-4'>Booking Detail</TableHead>
                <TableHead className='py-4'>Route & Price</TableHead>
                <TableHead className='py-4'>Schedule</TableHead>
                <TableHead className='py-4'>Assigned People</TableHead>
                  ...
                  <TableRow key={item._id} className='hover:bg-white/[0.02] border-white/5 transition-colors'>
                    <TableCell className="ps-6 py-4">
                        <div className='flex flex-col gap-1'>
                          ...
                          <span className='text-[11px] text-blue-400 font-semibold flex items-center gap-1.5'>
                            <Microscope size={12} /> {item.test?.name}
                          </span>
                    ...
                    <TableCell className='py-4'>
                       <div className='flex flex-col gap-1'>
                          ...
                          <div className='flex items-center gap-1 text-[11px] font-bold text-green-400 mt-0.5'>
                             <Banknote size={12} /> {item.price} BDT
                          </div>
                       </div>
                    </TableCell>
                    ...
                    <TableCell className='py-4'>
                        {item.assigned_helper ? (
                           <div className='flex items-center gap-2'>
                              <div className='h-8 w-8 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-500 shadow-inner'>
                                <UserCheck size={14} />
                              </div>
                              <div className='flex flex-col'>
                                <span className='text-[11px] font-bold'>{item.assigned_helper.name}</span>
                                <span className='text-[9px] text-muted-foreground'>@{item.assigned_helper.username}</span>
                              </div>
                           </div>
                        ) : (
      ...
      <AssignHelperDialog 
        ...
        onSuccess={() => queryClient.invalidateQueries({ queryKey: ['diagnostic-bookings'] })}
      />
    </>
  )
}
